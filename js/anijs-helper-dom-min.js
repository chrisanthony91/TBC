!function(){
    ...
    function c(a){
        for(var b=a.length;b-->0;)AniJS.EventSystem.purgeAllNodes(a[b]),a[b].parentNode.removeChild(a[b])
    }
    function d(a,b){
    ...
    function h(a,c,d){
        var e=[],f="*";
        return e=0===d.length?[c.dataAniJSOwner]:"target"===d[0]?[a.currentTarget]:b(d[0]),d.length>1&&(f=d[1]),
    ...
    k.makeClassAction=function(a,b,c,d,e){
        for(var f=b.behaviorTargetList,g=0;g<f.length;g++)element=f[g],e=element,0===d?b.nodeHelper.addClass(e,c[0]):1===d?b.nodeHelper.removeClass(e,c[0]):b.nodeHelper.hasClass(e,c[0])?b.nodeHelper.removeClass(e,c[0]):b.nodeHelper.addClass(e,c[0]);
        b.hasRunned||b.run()
    },
    k.remove=function(a,d,e){
        var f=e.length,g=a.target,h=null;
        if(0===f)c([g]);
        else for(;f-->0;)h=b(e[f])||[],c(h);
        d.hasRunned||d.run()
    },
    k.clone=function(a,c,d){
        var f=d.length,g=a.target,h=c.eventTarget,i=null,j=k.cloneNodeHelper,l=1;
        if(0===f)e(h,g,l,j);
        else{
            if(1==f){
    ...
    k.ancestors=function(a,b,c){
        var e=h(a,b,c),f=e.elements,i=e.selector,j=[],k=[],l=0,m=0;
        for(l=f.length;l-->0;)
    ...
    k.children=function(b,c,d){
        for(var e=h(b,c,d),f=e.elements,g=e.selector,i=f.length,k=0,l=[],m=null;i-->0;)
        for(m=f[i].children;k<m.length;k++)a(m[k],g)&&j([m[k]],l);
        return l
    },
    k.cloneNodeHelper=function(a,b){
        if(null!==b){
            var c=a.cloneNode(!0);AniJS.purgeEventTarget(c),c.removeAttribute("id"),b.appendChild(c)
        }
    }
}
(window);